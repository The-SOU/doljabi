...
export default function FaceReveal({ onComplete }: FaceRevealProps) {
  ...
  // Generate aged faces on mount — 병렬
  useEffect(() => {
    ...
    const generateFaces = async () => {
      ...
      const promises = ages.map(async (age) => {
        setGenerationStatus(`${age}세 얼굴 생성 중... (${completed}/${ages.length})`);
        try {
          console.log(`[FaceReveal] ${age}세 이미지 생성 시작`);
          ...
          if (result) {
            ...
            console.warn(`[FaceReveal] ${age}세 이미지 null 반환됨`);
          }
        } catch (err) {
          completed++;
          console.error(`[FaceReveal] ${age}세 이미지 생성 에러:`, err);
        }
      });
